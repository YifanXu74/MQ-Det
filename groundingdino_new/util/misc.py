...
def nested_tensor_from_tensor_list(tensor_list: List[Tensor], image_sizes=None):
    if image_sizes is not None:
        # for glip
        if tensor_list[0].ndim == 3:
            mask = []
            for tensor ,size in zip(tensor_list, image_sizes):
                H, W = size
                mask_ = tensor.new_zeros(tensor.shape[1], tensor.shape[2])
                mask_[H:, :] = 1
                mask_[:, W:] = 1
                mask_ = (mask_ != 0)
                mask.append(mask_)
