...
from maskrcnn_benchmark.modeling.language_backbone.modeling_bert_new import GatedCrossAttentionBlock

# import ipdb

def dict_update(target_dict, new):
    out={}
...
class VLFuse(torch.nn.Module):
    ...
    def init_configs(self, cfg):
        # common params
        self.lang_model = os.path.basename(cfg.MODEL.LANGUAGE_BACKBONE.MODEL_TYPE)
    ...
    def forward(self, x):
        ...
        # return features_dict
        # x = dict_update(x, {"visual": fused_visual_features, "lang": fused_language_dict_features})
        x.update({"visual": fused_visual_features, "lang": fused_language_dict_features})
        return x

class SupportFuse(nn.Module):
    def __init__(self, cfg):
        super(SupportFuse, self).__init__()
        self.fuse = GatedCrossAttentionBlock(dim=cfg.MODEL.LANGUAGE_BACKBONE.LANG_DIM, cfg=cfg, enable_ffn=False)
    def forward(self, x):
        language_dict_features = x["lang"]
        language_feature = language_dict_features['hidden']
        augmented_vision = language_dict_features['augmented_vision']
        vision_attention_mask = language_dict_features['vision_attention_mask']

        if augmented_vision is not None:    
            language_dict_features['hidden'] = self.fuse(x=language_feature, vision=augmented_vision, attention_mask=vision_attention_mask)

        x.update({"lang": language_dict_features})
        return x
...
class VLDyHead(torch.nn.Module):
    def __init__(self, cfg):
        ...
        if os.path.basename(cfg.MODEL.LANGUAGE_BACKBONE.MODEL_TYPE) == "bert-base-uncased":
            lang_cfg = BertConfig.from_pretrained(cfg.MODEL.LANGUAGE_BACKBONE.MODEL_TYPE)
        elif os.path.basename(cfg.MODEL.LANGUAGE_BACKBONE.MODEL_TYPE) == "clip":
            lang_cfg = cfg
        else:
            lang_cfg = None
        ...
        # text-query fusion before first fusion
        if cfg.VISION_QUERY.QUERY_FUSION:
            self.query_fuse_qv_layer = SupportFuse(cfg)
        ...
        if self.cfg.MODEL.DYHEAD.FUSE_CONFIG.MLM_LOSS:
            if os.path.basename(cfg.MODEL.LANGUAGE_BACKBONE.MODEL_TYPE) == "clip":
    ...
    def forward(self, x, language_dict_features=None, embedding=None, swint_feature_c4=None):
        ...
        if self.cfg.VISION_QUERY.QUERY_FUSION:
            feat_inputs = self.query_fuse_qv_layer(feat_inputs)
