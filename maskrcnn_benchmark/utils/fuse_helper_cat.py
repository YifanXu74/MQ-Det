...
class BiAttentionBlockForCheckpoint(nn.Module):
    ...
    def forward(self, q0, q1, q2, q3, q4, l, attention_mask_l=None, dummy_tensor=None, cache=None, attention_mask_cache=None):
        ...
        if self.cfg.MODEL.DYHEAD.FUSE_CONFIG.SEPARATE_BIDIRECTIONAL:
            ...
            cache_num=cache.shape[1]
            visual_features_flatten=torch.cat([cache, visual_features_flatten], dim=1)

            new_v, new_l = self.single_attention_call(visual_features_flatten, l, attention_mask_l=attention_mask_l)
            # [bs, N, C] -> [bs, C, N]
            new_v = new_v.transpose(1, 2).contiguous()

            cache, visual_features_flatten =visual_features_flatten[:, :cache_num], visual_features_flatten[:, cache_num:]
        ...
        return visu_feat[0], visu_feat[1], visu_feat[2], visu_feat[3], visu_feat[4], lang_feat[0], lang_feat[1], lang_feat[2], lang_feat[3], lang_feat[4], cache
