...
def delete_previous_checkpoint(cfg):  
    max_to_keep = cfg.SOLVER.MAX_TO_KEEP  
    if os.path.exists(cfg.OUTPUT_DIR):          
        checkpoint_list=[name for name in os.listdir(cfg.OUTPUT_DIR) if name.endswith('.pth') and 'final' not in name and 'best' not in name and 'last' not in name and 'resume' not in name]
        if len(checkpoint_list)>max_to_keep:
            max_bits=len(checkpoint_list[0].split('.')[0].split('_')[-1])
            iter_list=[int(name.split('.')[0].split('_')[-1]) for name in checkpoint_list]
            ordered_list=sorted(iter_list)
            for iter in ordered_list[:-max_to_keep]:
                iter_name=str(iter).zfill(max_bits)
                weight_name='model_'+iter_name+'.pth'
                weight_path=str(Path(cfg.OUTPUT_DIR, weight_name))
                try:
...
class DetectronCheckpointer(Checkpointer):
    ...
    def save(self, name, **kwargs):
        ...
        if self.cfg.SOLVER.MAX_TO_KEEP != -1:
            delete_previous_checkpoint(cfg=self.cfg)
