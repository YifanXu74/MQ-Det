...
class LvisEvaluatorFixedAP(object):
    ...
    def update(self, predictions, is_cur_results=False):
        if is_cur_results:
            # only for debug
            cur_results = predictions
        else:
    ...
    def save_instances_results(self, save_path):
        instances = []
        for cat, inst in self.by_cat.items():
            instances = instances + inst
        instances = sorted(instances, key=lambda x: x["image_id"], reverse=False)
        print('saving instances results to ', save_path)
