...
class COCODataset(CocoDetection):
    def __init__(self, ann_file, root, remove_images_without_annotations, transforms=None, ignore_crowd=True,
        ...
        custom_category_ids = kwargs.get('custom_category_ids')
        if custom_category_ids is not None:
            new_ids = []
            for img_id in self.ids:
                if isinstance(img_id, str):
                    ann_ids = self.coco.getAnnIds(imgIds=[img_id], catIds=custom_category_ids, iscrowd=None)
                else:
                    ann_ids = self.coco.getAnnIds(imgIds=img_id, catIds=custom_category_ids, iscrowd=None)
                if len(ann_ids) > 0:
    ...
    def __getitem__(self, idx):
        ...
        return img, target, idx, img_path
